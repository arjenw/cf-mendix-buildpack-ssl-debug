...
from buildpack.datadog import _get_user_checks_dir, update_config
    ...
    def run(self, m2ee, database_config):
        if is_enabled():
            ...
            if self.is_producer_app:
                # kafka connect cfg
                os.makedirs(
                    os.path.join(_get_user_checks_dir(), "jmx_2.d"),
                    exist_ok=True,
                )
                kafka_connect_cfg = (
                ...
                kafka_connect_cfg_path = os.path.join(
                    _get_user_checks_dir(), "jmx_2.d", "conf.yaml"
                )
                write_file(
                ...
                # kafka streams cfg
                os.makedirs(
                    os.path.join(_get_user_checks_dir(), "jmx_3.d"),
                    exist_ok=True,
                )
                kafka_streams_cfg = (
                ...
                kafka_streams_cfg_path = os.path.join(
                    _get_user_checks_dir(), "jmx_3.d", "conf.yaml"
                )
                write_file(
