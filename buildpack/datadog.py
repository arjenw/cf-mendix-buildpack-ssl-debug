...
from buildpack import metrics_agent, util
...
def _get_service_from_tags():
    ...
    service_tags = sorted(
        OrderedDict(
            dict_filter(metrics_agent.get_tags(), ("app", "service")),
        ).items(),
        reverse=True,
    )
    if service_tags:
...
def _get_datadog_tags():
    tags = metrics_agent.get_tags()
    if not "service" in tags:
        # app and / or service tag not set
        tags["service"] = _get_service()
...
def _set_up_dd_java_agent(m2ee, jmx_config_files):
    ...
    m2ee.config._conf["m2ee"]["javaopts"].extend(
        [
            ...
            "-D{}={}".format("dd.jmxfetch.statsd.port", _get_statsd_port()),
        ]
    )
